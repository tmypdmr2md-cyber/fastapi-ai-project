...
def generate_keywords(promt: str) -> list[str]:
    ...
    end_promt = f"""Ты бренд-стратег и копирайтер. На входе только название проекта/бизнеса: "{promt}". Сгенерируй 5 релевантных ключевых слов для этого проекта. 
    Формат: ровно одна строка без переносов и лишних слов; только значения через запятую с пробелом. Пиши по-русски, без клише,
    без подписей и лишних символов, не пиши хэштеги, не склеивай слова (пиши словосочетания через пробел), не используй аббревиатуры""".strip()
